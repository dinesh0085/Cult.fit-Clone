...
function Care({setc}) {
  ...
  return (
    ...
    <Box w={{ base: '100%', md: '95%', lg: '90%' }} m="auto">
        <Text fontSize={{ base: '14px', md: '18px', lg: '25px' }} marginTop="20px" fontWeight={"semibold"} color="black" textAlign="left">Lab Tests</Text>
        <Text fontSize={{ base: '12px', md: '14px', lg: '16px' }} color="grey" textAlign="left">Assess and monitor your health from the comfort of your home now.</Text> 
         ...
         {care && care.map((el,i)=>{
          if(i>=0 ){
           ...
           <Link to={`/details/${el._id}`}> <Image borderRadius="5px" src={el.image}></Image></Link> 
             ...
             {/*  price box */}
            <Box fontSize={{ base: '10px', md: '10px', lg: '12px' }} color={"grey"} display="flex" justifyContent="space-between">
              <Text color="black">Price</Text>
              <Text textDecoration="line-through" color="grey">₹ {el.price}</Text>
            </Box>
             {/*  price box */}
...
{/*-------------------- FAQ part -------------------*/}
     ...
     <Box marginTop={"30px"}  bgColor={'#171a26'} paddingTop="70px" paddingBottom={"50px"}>
       
       <Box  w={{ base: '100%', md: '95%', lg: '90%' }} m="auto" >
       
       <Heading color={"white"} textAlign={"left"} fontSize={["26px","32px","40px","48px"]} >FAQS</Heading>
       <Heading fontSize={{ base: '12px', md: '14px', lg: '18px' }} color={"grey"} marginTop="5px" textAlign={"left"}>7 QUESTIONS</Heading>
  ...
  <AccordionItem>
    ...
    <AccordionPanel pb={4} textAlign="left">
    The appointment can be rescheduled or cancelled 2 hours before your chosen slot.  
    </AccordionPanel>
  </AccordionItem>
  ...
  <AccordionItem>
    ...
    <AccordionPanel pb={4} textAlign="left">
    You can email your test reports by going to Profile {">"} Activity &Records{">"} Medical Records{">"} Diagnostic Tests and selecting the email reports button.   
    </AccordionPanel>
  </AccordionItem>
  ...
  <AccordionItem>
    ...
    <AccordionPanel pb={4} textAlign="left">
    All instructions that have to be followed for your tests will be communicated to you at the time of booking a slot. You will also receive an email covering the instructions.
        </AccordionPanel>
  </AccordionItem>
  ...
  <AccordionItem>
    <h2>
      <AccordionButton>
        <Box as="span" flex='1' textAlign='left' fontSize={["12px","16px","20px","24px"]}>
        Q6:- How can I get a refund for my Full body Health Check /Diagnostic Screening pack ?
        </Box>
        <AccordionIcon />
      </AccordionButton>
    </h2>
